...
import (
	...
	systemRes "fcas_server/model/system/response"
	"fmt"
	"gorm.io/gorm"
)
...
func (apiService *ApiService) CreateApi(api system.SysApi) (err error) {
	if !errors.Is(global.SystemDB.Where("path = ? AND method = ?", api.Path, api.Method).First(&system.SysApi{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("存在相同api")
	}
	return global.SystemDB.Create(&api).Error
}

func (apiService *ApiService) GetApiGroups() (groups []string, err error) {
	err = global.SystemDB.Model(&system.SysApi{}).Select("DISTINCT api_group").Pluck("api_group", &groups).Error
	return
}
...
func (apiService *ApiService) SyncApi() (newApis, deleteApis, ignoreApis []system.SysApi, err error) {
	...
	err = global.SystemDB.Find(&apis).Error
	if err != nil {
		return
	}
	var ignores []system.SysIgnoreApi
	err = global.SystemDB.Find(&ignores).Error
	if err != nil {
		return
	}
	...
	for i := range global.ROUTERS {
		ignoresFlag := false
		for j := range ignores {
			if ignores[j].Path == global.ROUTERS[i].Path && ignores[j].Method == global.ROUTERS[i].Method {
				ignoresFlag = true
			}
		}
		if !ignoresFlag {
			cacheApis = append(cacheApis, system.SysApi{
				Path:   global.ROUTERS[i].Path,
				Method: global.ROUTERS[i].Method,
			})
		}
	}
...
func (apiService *ApiService) IgnoreApi(ignoreApi system.SysIgnoreApi) (err error) {
	if ignoreApi.Flag {
		return global.SystemDB.Create(&ignoreApi).Error
	}
	return global.SystemDB.Unscoped().Delete(&ignoreApi, "path = ? AND method = ?", ignoreApi.Path, ignoreApi.Method).Error
}
...
func (apiService *ApiService) DeleteApi(api system.SysApi) (err error) {
	...
	err = global.SystemDB.First(&entity, "id = ?", api.ID).Error // 根据id查询api记录
	if errors.Is(err, gorm.ErrRecordNotFound) {                  // api记录不存在
		return err
	}
	err = global.SystemDB.Delete(&entity).Error
	if err != nil {
		return err
	}
...
//@description: 分页获取数据,
//@param: api model.SysApi, info req.PageInfo, order string, desc bool
...
func (apiService *ApiService) GetAPIInfoList(api system.SysApi, info request.PageInfo, order string, desc bool) (list interface{}, total int64, err error) {
	...
	db := global.SystemDB.Model(&system.SysApi{})
...
func (apiService *ApiService) GetApiById(id int) (api system.SysApi, err error) {
	err = global.SystemDB.First(&api, "id = ?", id).Error
	return
}
...
func (apiService *ApiService) UpdateApi(api system.SysApi) (err error) {
	...
	err = global.SystemDB.First(&oldA, "id = ?", api.ID).Error
	if oldA.Path != api.Path || oldA.Method != api.Method {
		var duplicateApi system.SysApi
		if ferr := global.SystemDB.First(&duplicateApi, "path = ? AND method = ?", api.Path, api.Method).Error; ferr != nil {
	...
	return global.SystemDB.Save(&api).Error
}
