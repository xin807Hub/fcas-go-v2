...
func (jwtService *JwtService) JsonInBlacklist(jwtList system.JwtBlacklist) (err error) {
	err = global.SystemDB.Create(&jwtList).Error
	if err != nil {
		return
	}
...
func (jwtService *JwtService) IsBlacklist(jwt string) bool {
	...
	return ok
	// err := global.SystemDB.Where("jwt = ?", jwt).First(&system.JwtBlacklist{}).Error
...
func LoadAll() {
	var data []string
	err := global.SystemDB.Model(&system.JwtBlacklist{}).Select("jwt").Find(&data).Error
	if err != nil {
