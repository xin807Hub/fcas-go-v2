...
func RegisterApis(apis ...system.SysApi) {
	...
	global.SystemDB.Find(&[]system.SysApi{}, "path in (?)", apiPaths).Count(&count)
	if count > 0 {
		fmt.Println("插件已安装或存在同名路由")
		return
	}
	err := global.SystemDB.Create(&apis).Error
	if err != nil {
		fmt.Println(err)
	}
}
...
func RegisterMenus(menus ...system.SysBaseMenu) {
	...
	global.SystemDB.Find(&[]system.SysBaseMenu{}, "name in (?)", menuNames).Count(&count)
	if count > 0 {
		fmt.Println("插件已安装或存在同名菜单")
		return
	}
	parentMenu.ParentId = 0
	err := global.SystemDB.Create(&parentMenu).Error
	if err != nil {
		fmt.Println(err)
	}
	...
	err = global.SystemDB.Create(&otherMenus).Error
	if err != nil {
		fmt.Println(err)
	}
}
