...
// TranslateErr 将错误翻译成中文
func TranslateErr(err error) string {
	var result = make(map[string][]string)
	switch err.(type) {
	case validator.ValidationErrors:
		errorStr := err.(validator.ValidationErrors)
		for _, e := range errorStr {
			result[e.Field()] = append(result[e.Field()], e.Translate(global.Trans))
		}
	case *json.UnmarshalTypeError:
		// json: cannot unmarshal number into Go struct field ReqParam.linkIdList of type []uint32
