    ...
    <div class="">
      ...
      >
        ...
        >
          ...
          <!-- 用户类型-->
          <el-form-item label="用户类型" label-width="120px" prop="user_type">
            <el-select v-model="form.user_type"
                       :placeholder="`请选择用户类型`"
                       class="w-112"
                       clearable
                       @change="handleChange($event, 'user_type')"
            >
              <el-option
          ...
          <el-form-item
              ...
              label="用户群组"
              label-width="120px"
              prop="user_crowd_group_id"
          >
            <el-select
                v-model.number="form.user_crowd_group_id"
                :placeholder="`请选择用户群组`"
                class="w-112"
                clearable
                @change="handleChange($event, 'user_crowd_group_id')"
            >
              <el-option
          ...
          <el-row :gutter="40">
            <el-col :span="12">
              <el-form-item label="应用大类" label-width="120px" prop="app_type_id">
                <el-select
...
function isAnyFieldValid(...fields) {
  return fields.some(field => {
    const flag = field !== null &&
        field !== undefined &&
        field !== 0 &&
        field !== '';

    // if (flag)  console.log('isAnyFieldValid', field)
...
const validateUser = (rule, value, callback) => {
  ...
  const user_crowd_group_id = form.value.user_crowd_group_id
  const user_crowd_id = form.value.user_crowd_id
  const user_id = form.value.user_id

  if (isAnyFieldValid(value, appTypeId, user_crowd_group_id, user_crowd_id, user_id)) {
    // console.log('至少有一个字段是有效值');
    callback();
  } else {
...
const validateApp = (rule, value, callback) => {
  ...
  if (isAnyFieldValid(value, userType, user_crowd_group_id, user_crowd_id, user_id)) {
    // console.log('至少有一个字段是有效值');
    callback();
  } else {
    callback(new Error('应用大小类与用户至少要选择一个'))
...
const validateInt = (rule, value, callback) => {
  ...
  if (value !== null) {
    if (!regex.test(value)) {
      callback(new Error('数值范围应在0-255!'))
    } else {
      callback();
    }
  } else {
...
const rules = ref({
  ...
  ul_tos: [
    {message: '下行tos与上行tos必须有一个不为空', validator: validateTos, trigger: 'blur'},
    // {message: '必须为数字!', type: 'number'},
    {message: '数值最大为15!', max: 15, type: 'number', trigger: 'blur'},
    {message: '数值最小为0!', min: 0, type: 'number', trigger: 'blur'},
  ],
  dl_tos: [
    {message: '下行tos与上行tos必须有一个不为空', validator: validateTos, trigger: 'blur'},
    // {message: '必须为数字!', type: 'number'},
...
const handleSubmit = async () => {
  formRef.value.validate(async valid => {
    ...
    if (valid) {
      ...
      // console.log('user_id', form.value['user_id'])
