    ...
    <div class="">
      ...
      >
        ...
        >
          ...
          <!--            用户-->
          <el-form-item label="用户" label-width="120px" prop="user_id">
            <el-select
              ...
              <el-table :data="IpData">
                ...
                <el-table-column prop="option" label="操作" fixed="right" width="100">
                  <template #default="scope">
                    <el-button
                      link type="danger" icon="Delete" title="删除" size="large"
                      @click="handleDelFromTable(scope.row, scope.$index)"
                    />
                  </template>
                </el-table-column>
              </el-table>
          ...
          <!--            周期类型-->
          <el-form-item label="周期类型" label-width="120px" prop="period_type">
            <el-select
...
const tableColumns = ref([
  ...
  {name: "下行丢弃流速", field: "dn_discard_speed"},
])
const timeRange = ref()
...
const handleKey = (str, val, field) => {
  searchEl.value.forEach(item => {
    if (item.field === "userId") {
      item.options = userOptions.value
    }
  })
...
onBeforeMount(async () => {
  ...
  for (const item of configtable.value) {
    if (item.name === "应用大类") {
    ...
    if (item.name === "用户") {
      ...
      let tmpOption = []
      const res = await userListApi({page: 1, limit: 999})
      if (res.code === 0) {
      ...
      userOptions.value = tmpOption
      // tableColumns[2].options = tmpOption
      // console.log('用户下拉', item.options)
    }
  }
...
const editFunc = (str, row) => {
  // console.log('control policy editFunc', row)
...
const handleSubmit = async () => {
  try{
    ...
    if (form.value.flow_ctrl_type === 2) {
      form.value.ip_data = IpData.value
    }
    ...
    const res = await control_createPolicyApi(form.value)
    if (res.code === 0) {
