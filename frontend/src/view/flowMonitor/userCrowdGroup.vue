  ...
  <div class="space-y-2">
    <el-card>
      <div class="inline-flex w-full gap-2 justify-self-center">
        ...
        <user-tree-select v-model:selected="searchForm.groupIdList" tree-type="groupTree" />
...
import { userCrowdGroupRankLevel1PieApi, userCrowdGroupRankLevel1TableApi, userCrowdGroupRankLevel2TableApi, userCrowdGroupRankLevel2TrendApi, userCrowdGroupRankLevel3TableApi } from "@/api/traffic";

const searchForm = ref({ startTime: dayjs().subtract(1, "hour").format("YYYY-MM-DD HH:mm:ss"), endTime: dayjs().format("YYYY-MM-DD HH:mm:ss"), ispNameList: [], linkIdList: [], groupIdList: [], appIdList: [] });
...
const fetchLevel1PieChartData = async () => {
  ...
  try {
    const res = await userCrowdGroupRankLevel1PieApi({ ...searchForm.value });
    if (res.code === 0) {
...
const fetchLevel1TableData = async () => {
  ...
  try {
    const res = await userCrowdGroupRankLevel1TableApi({ ...searchForm.value });
    if (res.code === 0) {
...
const loadLevel2TrendData = async (row) => {
  ...
  try {
    const res = await userCrowdGroupRankLevel2TrendApi({ startTime: searchForm.value.startTime, endTime: searchForm.value.endTime, groupIdList: [row.id], topN: 20 });
...
const loadLevel2TableData = async (row) => {
  ...
  try {
    const res = await userCrowdGroupRankLevel2TableApi({ startTime: searchForm.value.startTime, endTime: searchForm.value.endTime, groupIdList: [row.id], topN: 20 });
...
const handleOpenLevel3 = async (row) => {
  ...
  try {
    ...
    const res = await userCrowdGroupRankLevel3TableApi({ startTime: searchForm.value.startTime, endTime: searchForm.value.endTime, srcIp: row.srcIp });
...
const handleSearch = async () => {
  ...
  if (level3PanelVisible.value && selectedLevel3Ip.value) {
    const res = await userCrowdGroupRankLevel3TableApi({ startTime: searchForm.value.startTime, endTime: searchForm.value.endTime, srcIp: selectedLevel3Ip.value });
    level3TableData.value = res.code === 0 ? res.data || [] : [];
  }
};

const handleExport = async () => {
  axios({ method: "post", url: "/api/traffic/userCrowdGroupRank/export", data: searchForm.value, responseType: "blob" })
    .then((response) => {
