<template>
  <div class="space-y-2">
      ...
      <div class="bg-white rounded-xl p-2">
        ...
        >
          ...
          <template #totalRatio="{ row }">
            {{ formatTotalRatio(row.trafficTotal) }}
          </template>
        </ElTablePro>

        <div
          class="mt-2 ml-2 inline-flex flex-wrap items-center gap-5 text-gray-500"
        >
    ...
    <div
      ...
      ref="level2PanelRef"
      class="rounded-md bg-white p-3 shadow-md space-y-3 dark:bg-slate-900"
    >
      <p class="text-2xl font-black">
        [ {{ selectedLevel2UserName }} ] - 用户详情
      </p>
      ...
      <div class="rounded-md border border-slate-200 p-2 dark:border-slate-700">
        ...
        <PieChart
          v-loading="level2TableLoading"
          :data="level2BarChartData"
          chart-type="bar"
          bar-direction="horizontal"
          name-key="srcIp"
          value-key="trafficTotal"
          :name-max-length="18"
          :height="520"
          :show-y-axis="true"
      ...
      <div class="rounded-md border border-slate-200 p-2 dark:border-slate-700">
        ...
        >
          ...
          <template #totalProportion="{ row }">
            {{ ((row.totalProportion || 0) * 100).toFixed(4) }}%
          </template>
        </ElTablePro>
      </div>
    </div>
    ...
    >
      ...
      <div class="rounded-md border border-slate-200 p-2 dark:border-slate-700">
        ...
        <TrendChart
          v-loading="level3Loading"
          :data="level3TableData"
          x-axis-key="startTime"
...
import {
  userRankLevel1PieApi,
  userRankLevel1TableApi,
  userRankLevel2TableApi,
  userRankLevel2TrendApi,
  userRankLevel3TableApi,
} from "@/api/traffic";
...
const level2BarChartData = computed(() =>
  level2TableData.value.slice().sort((a, b) => a.trafficTotal - b.trafficTotal),
);
...
function formatTotalRatio(trafficTotal) {
  return `${(
    ((trafficTotal || 0) / (level1TableState.value.trafficTotal || 1)) *
    100
  ).toFixed(4)}%`;
}
...
const fetchLevel1PieChartData = async () => {
  ...
  try {
    const res = await userRankLevel1PieApi({ ...searchForm.value });
    if (res.code === 0) {
      level1PieChartData.value.trafficTotal =
        res.data?.map((item) => ({
          name: item.name,
          value: item.trafficTotal,
        })) || [];

      level1PieChartData.value.trafficDown =
        res.data?.map((item) => ({
          name: item.name,
          value: item.trafficDn,
        })) || [];
    }
  } finally {
...
const fetchLevel1TableData = async () => {
  ...
  try {
    const res = await userRankLevel1TableApi({ ...searchForm.value });
    if (res.code === 0) {
      const rows =
      ...
      level1TableState.value.rows = rows;
      ...
      const formatConfig = {
        ...
        trafficUp: { label: "上行总量" },
        trafficDn: { label: "下行总量" },
        trafficTotal: { label: "上下行总量" },
      };
      const gather = res.data?.gather || {};
      level1TableState.value.gather = Object.entries(formatConfig).reduce(
        (acc, [key, value]) => {
          acc[value.label] = formatSize(gather[key], value);
          return acc;
        },
        {},
      );
...
const loadLevel2TrendData = async (row) => {
  ...
  try {
    ...
    const res = await userRankLevel2TrendApi(params);
...
const loadLevel2TableData = async (row) => {
  ...
  try {
    ...
    const res = await userRankLevel2TableApi(params);
    level2TableData.value =
      res.code === 0
        ? (res.data || []).map((item) => ({
            ...item,
            level3Loading: false,
...
const handleOpenLevel3 = async (row) => {
  ...
  try {
    ...
    const res = await userRankLevel3TableApi(params);
...
const handleSearch = async () => {
  ...
  if (level2PanelVisible.value && selectedLevel2UserId.value !== null) {
    const currentRow = level1TableState.value.rows.find(
      (item) => item.id === selectedLevel2UserId.value,
    );
    if (currentRow) {
      await Promise.all([
        loadLevel2TrendData(currentRow),
        loadLevel2TableData(currentRow),
      ]);
    }
  }
