  ...
  <div class="space-y-2">
    <el-card>
      <div class="inline-flex w-full gap-2 justify-self-center">
        ...
        <app-type-select
          v-model:selected="searchForm.appTypeIdList"
    ...
    >
      <p class="text-2xl font-black">[ {{ selectedAppTypeName }} ] - 详情</p>
...
import { appTypeRankDataApi, appTypeRankTableDataApi } from "@/api/traffic";
...
const selectedAppTypeId = ref(null);
const selectedAppTypeName = ref("");
...
const level1Columns = computed(() => [
  ...
  { prop: "name", label: "业务大类", minWidth: 180 },
  {
    type: "bps",
    prop: "maxUpBps",
    label: "上行峰值",
...
const fetchLevel1TableData = async () => {
  ...
  try {
    ...
    const resp = await appTypeRankTableDataApi(params);
    if (resp.code === 0) {
...
const fetchLevel1PieChartData = async () => {
  ...
  try {
    const res = await appTypeRankDataApi({ ...searchForm.value });
    if (res.code === 0) {
...
const fetchLevel2TrendData = async () => {
  ...
  try {
    const params = {
      ...
      appTypeIdList: [selectedAppTypeId.value],
    };
    const resp = await appTypeRankDataApi(params);
...
const handleOpenLevel2 = async (row) => {
  ...
  try {
    selectedAppTypeId.value = row.appType;
    selectedAppTypeName.value = row.name;
...
const handleSearch = async () => {
  ...
  if (level2PanelVisible.value && selectedAppTypeId.value !== null) {
    level2Pagination.value.page = 1;
    await Promise.all([fetchLevel2TrendData(), fetchLevel2TableData()]);
  }
};
...
const handleExport = async () => {
  axios({
    ...
    .then((response) => {
      const filename = `app_type_rank_${+new Date()}.xlsx`;
      saveAs(
