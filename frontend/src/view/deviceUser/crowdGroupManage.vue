...
// 引入接口
import {createUserCrowdGroupApi, updateUserCrowdGroupApi, deleteUserCrowdGroupApi, userCrowdApi} from "@/api/userMng"
...
const searchEl = ref([
  {name: "用户群组名称", field: "key"}
])
// 列表查询逻辑
const loadKey = ref(0)
const tableParam = ref(null)
const tableColumns = ref([
  {name: "用户群组名称", field: "group_name"},
  {name: "关联用户群", field: "crowds"},
...
const configItems = ref([
  {name: "用户群组名称", field: "group_name", type: "string"},
  {name: "备注", field: "remark", type: "textarea"},
  {name: "关联用户群", field: "crowdIds", type: "transfer"},
])
const rules = ref({
  group_name: [{required: true, message: '请输入用户群组名称', trigger: 'blur'}],
})
// 穿梭框数据
const bindData = ref([])
const data = ref([])
const handleChange = (value) => {
  form.value.crowdIds = value
}
const getUserData = async () => {
  // if (localStorage.getItem('userCrowdData')) {
  //   data.value = JSON.parse(localStorage.getItem('userCrowdData'))
  // } else {
  const res = await userCrowdApi({
    page: 1,
  ...
  if (res.code === 0) {
    ...
    // localStorage.setItem('userCrowdData',JSON.stringify(temp_data))
...
const editFunc = (str, row) => {
  ...
  bindData.value = row.crowds.map(item => item.id)
}

const deleteFunc = async (str, row) => {
  const res = await deleteUserCrowdGroupApi([row.id])
  if (res.code === 0) {
...
const handleSubmit = async () => {
  formRef.value.validate(async valid => {
    if (valid) {
      form.value.groupName = form.value.group_name
      let res = {}
      if (configTitle.value === "新建") {
        res = await createUserCrowdGroupApi(form.value)
      } else {
        res = await updateUserCrowdGroupApi(form.value)
      }
      if (res.code === 0) {
