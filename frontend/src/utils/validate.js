// 是否合法IP地址
export function validateIPVal(value) {
...
export function validatorBytes(str, min = 1, max = 128) {
...
export function validateDomainVal(value) {
...
export function validateDomain(rule, value, callback) {
  ...
  if (value !== null) {
    if ((!reg.test(value)) && value !== '') {
      callback(new Error('请输入正确的DNS域名'))
    } else {
      callback()
    }
  } else {
...
export function validateURL(rule, value, callback) {
  const strRegex = /^(https?|ftp):\/\/([a-zA-Z0-9.-]+(:[a-zA-Z0-9.&%$-]+)*@)*((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]?)(\.(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])){3}|([a-zA-Z0-9-]+\.)*[a-zA-Z0-9-*]+\.(com|edu|gov|int|mil|net|org|biz|arpa|info|name|pro|aero|coop|museum|[a-zA-Z]{2}))(:[0-9]+)*(\/($|[a-zA-Z0-9.,?'\\+&%$#=~_-]+))*$/
 ...
 * IP地址校验，支持ipv4和ipv6
 */
export function validIpAddress(rule, value, callback, limit) {
  if (value) {
    ...
    if (limit !== undefined) {
      if (total > limit) {
        callback(new Error('不超过' + limit + '条'))
      }
    }
    if (arr.length > 1) {
      var sarr = arr.sort()
...
export function isValidUint32(value) {
  // 检查值是否为整数，并且在0 - 4,294,967,295 的范围内
