  ...
  <div>
    <el-card class="mt-2">
      ...
      >
        ...
        <el-table-column v-if="props.operation" type="selection" fixed="left" width="50"/>
        ...
        <el-table-column
            ...
            :formatter="colFormatter"
        >
          <template v-if="item.link===true" #default="scope">
            <el-link :icon="Link" type="primary" @click="handleLinkClick(scope.$index, scope.row)">
              {{ scope.row[item.field] }}
            </el-link>
          </template>
          <template v-if="item.field.indexOf('Time') > - 1" #default="scope">
          ...
          <template v-else-if="item.field==='crowds'" #default="scope">
            <el-tag v-for="crowd in scope.row[item.field]" type="success" size="small" class="mr-1">
              {{ crowd.crowdName }}
            </el-tag>
          </template>
          <template v-else-if="item.field==='users'" #default="scope">
            <el-tag v-for="user in scope.row[item.field]" type="success" size="small" class="mr-1">
              {{ user.userName }}
            </el-tag>
          </template>
          <template v-else-if="item.field==='ipAddress'" #default="scope">
            <el-tag v-for="ipAddress in scope.row[item.field]" type="success" size="small" class="mr-1">
              {{ ipAddress }}
            </el-tag>
          </template>
          <template v-else-if="item.field.indexOf('Ip') > -1 && item.field !== 'srcIp'" #default="scope">
            <el-tag type="primary" size="small" effect="dark" class="mr-1">
              {{ scope.row[item.field] }}
            </el-tag>
          </template>
          <template v-else-if="item.field==='linkState'" #default="scope">
            <el-tag v-if="scope.row[item.field] === 0" type="danger" size="small">
              断开
            </el-tag>
            <el-tag v-else-if="scope.row[item.field] === 1" type="success" size="small">
              正常
            </el-tag>
          </template>
          <template v-else-if="item.field==='totalRatio'" #default="scope">
            {{
          ...
          <template v-else-if="item.field==='totalProportion'" #default="scope">
            {{ (scope.row[item.field] * 100).toFixed(4) + "%" }}
          </template>
          <template v-else-if="item.field==='diskUseMap'" #default="{row}">
            <div v-for="(value, key) in row.diskUseMap" :key="key">
              <el-tag type="success">{{ key }}: {{ (value[1] / value[0] * 100).toFixed(2) }}%</el-tag>
            </div>
          </template>
          <template v-else-if="item.field==='processMap'" #default="{row}">
            <div v-for="(value, key) in row.processMap" :key="key">
              <el-tag v-if="value!=0" type="success">{{ key }}</el-tag>
            </div>
          </template>
        </el-table-column>
        <el-table-column v-if="props.operation" prop="option" label="操作" fixed="right" width="100">
          <template #default="scope">
            <el-button link type="success" icon="Edit" title="编辑" size="large" @click="handleEdit(scope.row)"/>
            <el-button link type="danger" icon="Delete" title="删除" size="large" @click="handleDelete(scope.row)"/>
          </template>
        </el-table-column>
      </el-table>
...
import {formatByte, formatIsOversea} from '@/utils/format'
// 引入分页组件
...
const colFormatter = (row, column) => {
  if (column.property.indexOf("time") && column.property === "update_at") {
    ...
    return formatByte(row[column.property], "Mbps")
  } else {
