<template>
  <div>
    ...
    <CommonTable
        ...
        :auto-height="true"
        module-name="traffic"
        module-api="userCrowdRankLevel1TableApi"
    ...
    >
      ...
      <div :key="loadKey2" class="w-full h-176 overflow-y-scroll">
        ...
        <ReTrend
            ...
            module-name="traffic"
            module-api="userCrowdRankLevel2TrendApi"
        />
        ...
        <ReTop
            ...
            :line="false"
            module-name="traffic"
            module-api="userCrowdRankLevel2TableApi"
        />
        <!--    二级列表    -->
        <CommonTable
            v-if="toolbarParam!==null"
...
// 引入接口
import { dictInfoApi, userCrowdRankLevel1PieApi} from "@/api/traffic";
// 状态管理
...
const getPieData = async () => {
  pieData.value = []
  gatherData.value = []
  const res = await userCrowdRankLevel1PieApi(toolbarParam.value.form)
  if (res.code === 0) {
...
// 查询
const searchFunc = () => {
  toolbarParam.value.form.crowdIdList = toolbarParam.value.form.copyIdList.map(item => {
...
const exportFunc = async () => {
  axios({
    method: "post",
    url: '/api/traffic/userCrowdRank/export',
    data: toolbarParam.value.form,
    responseType: 'blob',
  })
      .then((response) => {
        const filename = `userCrowd_rank_${+new Date()}.xlsx`
...
const linkClickFunc = (str, index, row) => {
  ...
  toolbarParam.value.form.crowdIdList = toolbarParam.value.form.copyIdList
...
onBeforeMount(async () => {
  ...
  // 获取用户群树
  crowdOptions.value[0] = await userTreeStore.getCrowdTree()
  ...
  searchEl.value.forEach(item => {
    if (item.name === "运营商") {
      ...
      item.treeOptions = renameIdToValue(crowdOptions.value)
