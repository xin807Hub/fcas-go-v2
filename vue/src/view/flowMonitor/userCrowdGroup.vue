<template>
  <div>
    ...
    <CommonTable
        ...
        :auto-height="true"
        module-name="traffic"
        module-api="userCrowdGroupRankLevel1TableApi"
    ...
    >
      ...
      <div :key="loadKey2" class="w-full h-176 overflow-y-scroll">
        ...
        <ReTrend
            ...
            module-name="traffic"
            module-api="userCrowdGroupRankLevel2TrendApi"
        />
        ...
        <ReTop
            ...
            :line="false"
            module-name="traffic"
            module-api="userCrowdGroupRankLevel2TableApi"
        />
        <!--    二级列表    -->
        <CommonTable
            v-if="toolbarParam!==null"
...
// 引入接口
import {dictInfoApi, userCrowdGroupRankLevel1PieApi} from "@/api/traffic";
// 状态管理
...
const searchEl = ref([
  ...
  {name: "用户/群", field: "copyIdList", treeOptions: [], multiple: true, width: 'w-60', lazy: false},
...
const getPieData = async () => {
  pieData.value = []
  gatherData.value = []
  const res = await userCrowdGroupRankLevel1PieApi(toolbarParam.value.form)
  if (res.code === 0) {
...
const searchFunc = () => { // 搜索
  toolbarParam.value.form.groupIdList = toolbarParam.value.form.copyIdList.map(item => {
...
const exportFunc = async () => {
  axios({
    method: "post",
    url: '/api/traffic/userCrowdGroupRank/export',
    data: toolbarParam.value.form,
    responseType: 'blob',
  })
      .then((response) => {
        const filename = `userCrowdGroup_rank_${+new Date()}.xlsx`
...
const linkClickFunc = (str, index, row) => {
  ...
  toolbarParam.value.form.groupIdList = toolbarParam.value.form.copyIdList
...
onBeforeMount(async () => {
  ...
  // 获取用户组群树
  crowdGroupOptions.value[0] = await userTreeStore.getGroupTree()
  ...
  searchEl.value.forEach(item => {
    ...
    } else if (item.name === "用户/群") {
      item.treeOptions = renameIdToValue(crowdGroupOptions.value)
