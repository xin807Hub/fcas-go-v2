...
<template>
  <div class="flex justify-between fixed top-0 left-0 right-0 z-10 bg-white text-slate-700 dark:text-slate-300  dark:bg-slate-900 shadow dark:shadow-gray-700 flex items-center px-2">
    <div
      class="flex items-center cursor-pointer flex-1"
    >
      <div class="flex items-center justify-center cursor-pointer" :style="{ width: config.layout_side_width + 'px' }" @click="router.push({ path: '/' })">
        <img
          alt
          class="h-8 bg-white dark:bg-slate-900 rounded-full"
          :src="$GIN_VUE_ADMIN.appLogo"
        >
        <div
          v-if="!isMobile"
          class="inline-flex font-bold text-xl ml-2"
