<template>
  ...
  >
    ...
    />
    <gva-header />
    <div class="flex flex-row w-full gva-container pt-12 box-border h-full">
      ...
      <div class="flex-1 w-0 h-full">
        ...
        <div
          class="overflow-auto bg-gray-50 dark:bg-slate-800"
          :class="config.showTabs ? 'gva-container2' : 'gva-container'"
        >
          <router-view v-if="reloadFlag" v-slot="{ Component,route }">
            <div
              id="gva-base-load-dom"
              class="gva-body-h dark:bg-slate-800 p-2"
            >
              <transition mode="out-in" name="el-fade-in-linear">
                <keep-alive :include="routerStore.keepAliveRouters">
                  <component :is="Component" :key="route.fullPath" class="py-0" />
                </keep-alive>
              </transition>
            </div>
          </router-view>
...
const { config, theme, menuTheme, device } = storeToRefs(appStore);
...
watchEffect(() => {
  font.color =
      (theme.value === "dark" || menuTheme.value === "dark") ? "rgba(255,255,255, .15)" : "rgba(0, 0, 0, .15)";
});
