...
export const useAppStore = defineStore('app', () => {
  ...
  const menuTheme = ref(localStorage.getItem('menuTheme')  || originSetting.menuDarkMode || 'auto')
  const device = ref("")
  ...
  if (localStorage.getItem('menuDarkMode')) {
    config.menuDarkMode = localStorage.getItem('menuDarkMode')
  }
  ...
  watchEffect(() =>{
    ...
    if (menuTheme.value === 'dark2'){
      document.documentElement.classList.add('dark2');
      document.documentElement.classList.remove('light');
      localStorage.setItem('menuTheme', 'dark2');
    }else{
        document.documentElement.classList.add('light');
  ...
  const toggleMenuTheme = (dark) => {
    if (dark) {
      menuTheme.value = 'dark2';
    } else {
  ...
  const toggleMenuDarkMode = (e) => {
    config.menuDarkMode = e
    localStorage.setItem('menuDarkMode', e)
    if(e === 'dark2'){
      toggleMenuTheme(true)
    }else{
      toggleMenuTheme(false)
    }
  }
